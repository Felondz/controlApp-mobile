...
>> **Last Updated**: May 15, 2026 - Real-time Notification Synchronization (v3.3.0)
...
13. [WebSockets / Tiempo Real](#websockets--tiempo-real)
...
```http
GET /api/proyectos/{uuid}
Authorization: Bearer {token}
Accept: application/json
```

**Parámetros**
- `uuid` - UUID del proyecto (string)
...
### List Notificaciones - Listar Notificaciones
Obtiene las notificaciones del usuario (vía GraphQL o REST).
*Nota: Se recomienda usar GraphQL para obtener el listado con metadatos.*

### Delete Notificación - Eliminar Notificación Individual
Elimina una notificación específica y sincroniza con otros dispositivos vía WebSockets.

```http
DELETE /api/notifications/{notification_uuid}
Authorization: Bearer {token}
Accept: application/json
```
...
### Clear All - Eliminar Todas las Notificaciones
Limpia la bandeja de entrada de notificaciones del usuario.
...
**Response (200)**: `{"status": "success", "message": "Todas las notificaciones han sido eliminadas."}`
...
**Endpoint Base**: `/api/mis-proyectos/{proyecto}/tasks`

### List Tareas - Listar Tareas del Proyecto
```http
GET /api/mis-proyectos/{proyecto}/tasks
Authorization: Bearer {token}
Accept: application/json
```

### Store Tarea - Crear Nueva Tarea
Soporta subida de imágenes múltiples vía `multipart/form-data`.
```http
POST /api/mis-proyectos/{proyecto}/tasks
Authorization: Bearer {token}
Content-Type: multipart/form-data
...
### Store Comment - Añadir Comentario
```http
POST /api/mis-proyectos/{proyecto}/tasks/{task}/comments
Authorization: Bearer {token}
Content-Type: application/json

{
  "content": "Hola @usuario, revisa esto."
}
```

---

## 📱 GraphQL API (Mobile Data Layer)

**Endpoint**: `/graphql` (POST)
**Estrategia**: >95% de las operaciones móviles se realizan aquí para optimizar el payload y las relaciones.

### Tareas (Mutations)

#### Crear Tarea con Asignados y Relaciones
```graphql
mutation {
  createTask(
    project_id: "1"
    title: "Tarea desde Mobile"
    status: "in_progress"
    priority: "high"
    assignees: ["1", "2"]
    related_type: "App\\Modules\\Operations\\Models\\LoteProduccion"
    related_id: "5"
  ) {
    id
    uuid
...
mutation {
  createTaskComment(
    task_id: "10"
    content: "Comentario desde la app móvil"
  ) {
    id
    content
    user {
