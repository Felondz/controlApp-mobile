...
import { View, Text, ScrollView, Switch, useWindowDimensions, TouchableOpacity, ActivityIndicator, InteractionManager } from 'react-native';
import { useTranslate, useAppTheme } from '../../../src/shared/hooks';
import { PuzzleIcon, CalculatorIcon, PackageIcon, ShoppingBagIcon, SparklesIcon } from '../../../src/shared/icons';
import { useAuthStore } from '../../../src/stores/authStore';
import { useProjectStore } from '../../../src/stores/projectStore';
import { useInventoryItems } from '../../../src/hooks/graphql/useInventory';
...
export default function MarketplaceScreen() {
    ...
    const [activeTab, setActiveTab] = useState<'tools' | 'products'>('tools');
    ...
    const tools = [
        ...
        { 
            id: 'ai-chat', 
            title: 'AI Assistant', 
            desc: 'Asistente inteligente para tus proyectos.', 
            icon: <SparklesIcon size={20} color="#f59e0b" />, 
            enabled: false,
            comingSoon: true
        },
    ];

    const { data: inventoryData, isLoading: loadingProducts } = useInventoryItems(activeProject?.id || '', {
        type: 'finished_good',
        is_active: true
    });
        ...
        <View className="flex-1 bg-secondary-50 dark:bg-secondary-950">
            ...
            >
                ...
                <View className="mt-4 mb-6 flex-row items-center justify-between">
                    ...
                    >
                        <ShoppingBagIcon size={24} color={theme.primary600} />
                    </View>
                </View>
                    ...
                    {tools.map((tool) => (
                        ...
                        >
                            <View 
                                ...
                                style={{ backgroundColor: `${tool.comingSoon ? '#f59e0b20' : theme.primary600 + '10'}` }}
                            >
                                {tool.icon}
                            </View>
                            
                            <Text className="text-base font-black text-secondary-900 dark:text-secondary-50 mb-1 leading-tight" numberOfLines={2}>
                                {tool.title}
                            </Text>
                            ...
                            <View className="flex-row items-center justify-between pt-3 border-t border-secondary-50 dark:border-secondary-800">
                                <Text className="text-[10px] font-black uppercase tracking-widest text-secondary-400">
                                    {tool.comingSoon ? 'Soon' : (tool.enabled ? t('common.active') : 'Off')}
                                </Text>
                                {!tool.comingSoon && (
                                    <Switch
                                        value={tool.enabled}
